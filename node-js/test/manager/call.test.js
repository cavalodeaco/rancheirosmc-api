...
describe('The /manager/call POST endpoint on production', function () {
  ...
  it('should call enrolls from the waiting list given class and an enroll list', async () => {
    ...
    ddbMock
      .on(GetCommand, { TableName: `${process.env.TABLE_NAME}-class` }).resolves({
        Item: {
      ...
      .on(GetCommand, { TableName: `${process.env.TABLE_NAME}-enroll` }).resolves({
        Item: { enroll_status: "waiting", ...enroll }
      })
      .on(UpdateCommand, { TableName: `${process.env.TABLE_NAME}-enroll` }).resolves({
        Item: { enroll_status: "called", ...enroll }
      });

    const { status, body: { message, enrolls } } = await request
      .post('/manager/call')
